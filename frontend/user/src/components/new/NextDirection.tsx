...
export interface CycleData {
  ...
  /**
   * 고른 이동 방향 (길이·컬러·앞머리·펌·회복·이미지). 예전엔 화면에만 남고 저장되지 않았다.
   * 기존 코스(components/3way/NextDirection)는 이 값을 보고하지 않으므로 선택 필드다.
   */
  directions?: DirectionOption[];
  /** 방향들의 평균 임팩트에서 나온 변화 강도 0~4. 0 = 안정 유지. 위와 같은 이유로 선택 필드. */
  changeLevel?: number;
}

/** 이동 방향 6개. 화면과 리포트 9페이지가 같은 표를 본다. */
...
export function NextDirection({ onBack, onNext, onCycleDataChange }: NextDirectionProps) {
  ...
  // 변화 강도 레벨
  const changeLevels = CHANGE_LEVELS;
  ...
  // 월별 시술·방향·강도가 바뀔 때마다 상위로 한 번에 보고한다.
  useEffect(() => {
    onCycleDataChange?.({ selectedMonths: cycleData, directions: selectedDirections, changeLevel });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cycleData, selectedDirections, changeLevel]);
